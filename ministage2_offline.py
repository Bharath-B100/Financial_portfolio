...
# Streamlit App
st.title("Tradebook Analysis (Offline Mode)")

# Fetch Tradebook Data from local file
def fetch_tradebook_entries():
    try:
        if os.path.exists('data/tradebook_entries.csv'):
            df = pd.read_csv('data/tradebook_entries.csv')
            df['quantity'] = pd.to_numeric(df['quantity'], errors='coerce')
            df['price'] = pd.to_numeric(df['price'], errors='coerce')
            df.dropna(subset=['quantity', 'price'], inplace=True)
            return df
        else:
            st.warning("No tradebook data found. Please run Stage 1 first to upload data.")
            return pd.DataFrame()
    except Exception as e:
        st.error(f"Error reading tradebook data: {e}")
        return pd.DataFrame()
...
if not df.empty:
    ...
    realized_pnl_summary_df = calculate_realized_pnl_summary(df)

    if not realized_pnl_summary_df.empty:
        st.subheader("Realized PnL Summary")
        total_realized_pnl = realized_pnl_summary_df["realised_pnl"].sum()
        st.dataframe(
            realized_pnl_summary_df.style.format({
                "qty": "{:.0f}",
                "buy_avg": "₹{:.2f}",
                "buy_value": "₹{:.2f}",
                "sell_avg": "₹{:.2f}",
        ...
        # Visualization
        fig = px.bar(realized_pnl_summary_df, x="symbol", y="realised_pnl", 
    ...
    st.write(f"Total Unique symbols: {df['symbol'].nunique()}")
    st.write(f"Total Transactions: {len(df)}")
else:
    st.warning("No tradebook entries found. Please run Stage 1 first to upload data.")
